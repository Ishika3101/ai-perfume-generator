...
//   const response = await fetch("https://ai-perfume-generator-3.onrender.com/generate", {
//     method: "POST",
//     headers: {
...
//               alt="composition chart"
//               className="mx-auto max-w-[260px]"
//             />
//           ) : (
//             <p className="text-sm text-gray-700">
//               Chart unavailable for this formula
//             </p>
//           )}
...
export default function PerfumeLab() {
  ...
  async function generatePerfume() {
    ...
    try {
      const response = await fetch("https://ai-perfume-generator-5.onrender.com/generate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-r ${activeTheme} p-10 transition-all duration-700`}>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        {/* Ingredient Selector */}
        <div className="bg-white/30 backdrop-blur-md p-6 rounded-xl shadow-md">
          <h2 className="text-xl font-bold mb-4">Ingredient Lab</h2>
          <IngredientSelector onSelect={setSelectedIngredients} />
        </div>

        {/* Fragrance Profile */}
        <div className="bg-white/30 backdrop-blur-md p-6 rounded-xl shadow-md">
          <h2 className="text-xl font-bold mb-4">Fragrance Profile</h2>
          ...
          <button
            ...
            className="w-full bg-black text-white py-3 rounded-xl hover:scale-[1.02] transition"
          >
            {loading ? "AI is crafting your perfume..." : "✨ Generate AI Fragrance"}
          </button>
        </div>
      ...
      {result && (
        <div className="mt-12 space-y-10">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            {/* Formula */}
            <div className="bg-white/30 backdrop-blur-md p-6 rounded-xl shadow-md">
              <h2 className="text-xl font-bold mb-4">Generated Formula</h2>
              <div className="space-y-2">
                {result.formula?.map((item, i) => (
                  <p key={i}>
                    {item.ingredient} — {item.percentage.toFixed(2)}% | {item.ml.toFixed(2)} ml
            ...
            {/* Chart */}
            <div className="bg-white/30 backdrop-blur-md p-6 rounded-xl shadow-md text-center">
              <h2 className="text-xl font-bold mb-4">Composition Chart</h2>
              {result.chart && (
  <img
    src={"data:image/png;base64," + result.chart}
    alt="chart"
